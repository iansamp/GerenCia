...
export default function FormBabidas() {
  ...
  const handleSubmit = () => {
    if (!values.nome || !values.valor || !values.tipo_no) {
      return;
    }

    Axios.post("http://localhost:3001/registerBebidas", values)
      .then((res) => {})
      .catch((err) => {
        console.error(err);
      });
  ...
  useEffect(() => {
    if (!categories) {
      Axios.get("http://localhost:3001/categoriasBebidas")
        .then((response) => {
